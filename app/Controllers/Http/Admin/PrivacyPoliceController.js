...
class PrivacyPoliceController {
    ...
    async delete({ response, params }) {
        const { id } = params
        let privacy_police = new ModelRepository(PrivacyPolice)
        let res = await privacy_police.deleteBy('uuid', id)
        if (!res) return response.Wrapper(500, false, "Delete privacy police failed, please try again later!")
        return response.Wrapper(200, true, "Delete privacy pilice data successfully!")
    }
}
